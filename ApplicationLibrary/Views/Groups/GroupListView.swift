...
public struct GroupListView: View {
    ...
    private func connect() {
        if ApplicationLibrary.inPreview {
            groups = [
                OutboundGroup(tag: "新加坡", type: "selector", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/SG.png",
                              selected: "server", selectable: true, isExpand: true, items: [
                ...
                OutboundGroup(tag: "group2", type: "urltest", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/US.png", selected: "client1", selectable: true, isExpand: false, items:
                    (0 ..< 100).map { index in
    ...
    private func setGroups(_ goGroups: [LibboxOutboundGroup]) {
        ...
        for goGroup in goGroups {
            ...
            groups.append(OutboundGroup(tag: goGroup.tag, type: goGroup.type, icon: goGroup.icon, selected: goGroup.selected,
                                        selectable: goGroup.selectable, isExpand: goGroup.isExpand, items: items))
        }
        self.groups = groups
        isLoading = false
    }
